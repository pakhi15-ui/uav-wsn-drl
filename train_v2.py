...
env = MultiUAVEnv(n_sensors=10, grid_size=100, max_steps=200, n_uavs=2)
model = PPO("MlpPolicy", env, learning_rate=1e-3, n_steps=1024,
            batch_size=128, n_epochs=15, gamma=0.995, verbose=1)

cb = RewardCallback()
print("Training Multi-UAV system with obstacles — 200,000 steps...")
model.learn(total_timesteps=200000, callback=cb, progress_bar=True)
model.save("models/trained_uav_agent_v2")
print("Saved to models/trained_uav_agent_v2")
...
if len(rewards) > 10:
    ...
    plt.title("Multi-UAV Training Progress"); plt.grid(True); plt.legend()
    plt.savefig("results/training_v2.png", dpi=150)
    plt.show()
